...
A public home page would not authorise real participation. These protections
still need engineering and a new rehearsal before invitations or real repair
publication:

- full replies are hidden when overdue, but physical deletion is triggered by
  the next site request rather than an independent scheduled job;
- the old outcome editor can publish immediately and does not prove that every
  private reply used in a nameless summary gave that optional permission; and
- repair and update publish checks are not yet tied to the exact revision the
  owner saw, so an older browser tab could approve newer unseen words.

Keep invitations, real Repair Receipts and real repair/update publication shut
until those three gates pass their own fake-data tests and owner-only deploy.
...
`npm run verify:release` covers only unit/database tests, lint, Drizzle's schema
check and the production build. It does not install dependencies, compare the
lockfile hash, inspect Git, deploy, inspect access, read the hosted environment
or check the live database. Record those as separate evidence. A clean locked
install, when claimed, must use `npm ci` in a disposable copy at the recorded
Node/npm versions.
